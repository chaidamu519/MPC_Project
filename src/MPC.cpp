...
std::vector<double> MPC::Solve(const VectorXd &state, const VectorXd &coeffs) {
  ...
  // Non-actuators upper and lowerlimits
  ...
  for (unsigned int i = delta_start; i < a_start; ++i) {
    vars_lowerbound[i] = -0.436332;    //-0.436332
    vars_upperbound[i] = 0.436332;     //0.436332
  }
   ...
   */
   ...
   // model output
   std::vector<double> actuate;
   
   // first actuator
   actuate.push_back(solution.x[delta_start]);
   actuate.push_back(solution.x[a_start]);
   
   // point positions
   for (unsigned i=0; i<N-1; i++){
	   actuate.push_back(solution.x[x_start + 1 + i]);
	   actuate.push_back(solution.x[y_start + 1 + i]);
	   }
