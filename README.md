...
The state includes the car's position (transformed to car's coordinate), orientation, velocity and then the cross-track error as well as the orientation error. These values are updated each dt (0.1 s used here).
...
Steering angle: -25 to +25 degrees.
Throttle: between +1 and -1, which corresponds to a full acceleration and a full brake.
...
One advantage of MPC is that it enalbes to add latency (100 ms in this project) in the model. This is taken into account each time before using the mpc model to find the new steering angle and the throttle. 
We have tested without adding the latency and the car can move properly at a much lower speed (15 or 20 mph). However, the car cannot pass certain sudden turns at higher speed.
